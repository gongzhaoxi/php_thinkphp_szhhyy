<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:109:"/mnt/datadisk0/www/wwwroot/wchuanghua2.ecloudm.com/public/../application/admin/view/series_process/index.html";i:1758808172;s:92:"/mnt/datadisk0/www/wwwroot/wchuanghua2.ecloudm.com/application/admin/view/public/header.html";i:1635496794;}*/ ?>
<!DOCTYPE html>
...
<div class="layui-card">
	<div class="layui-card-body">
		<form class="layui-form" action="">
			<div class=" layui-inline">
				<select name="process_id" xm-select="process_id"  xm-select-search  xm-select-search-type="dl" xm-select-radio>
					<option value="">工序</option>
					<?php foreach($process as $key=>$vo): ?>
					<option value="<?php echo $vo['id']; ?>"><?php echo $vo['name']; ?></option>
					<?php endforeach; ?>
				</select>	
			</div>
			<div class=" layui-inline" style="width:400px;">
				<select name="series_id" id="series_id" xm-select="series_id"  xm-select-search  xm-select-search-type="dl" xm-select-radio>
					<option value="">系列</option>
					<?php foreach($series as $key=>$vo): ?>
					<option value="<?php echo $vo['id']; ?>"><?php echo $vo['name']; ?></option>
					<?php endforeach; ?>
				</select>
			</div>					
	...
	layui.config({
		...
		select: "select",
	}).use(['table', 'form', 'jquery','select'],function() {
		...
		table.render({
			...
			cols: [[
				{type: 'checkbox'},
				{field: 'series_name',title: '系列',align: 'center'},
		...
		table.on('tool(dataTable)', function(obj) {
			if (obj.event === 'remove') {
				layer.confirm('确定要删除这些系列工序？',
					function(index) {
						ajaxform("<?php echo url('del'); ?>",{ids:[obj.data['id']]},3);
					}
				);				
		...
		table.on('toolbar(dataTable)', function(obj) {
			if (obj.event === 'add') {
				xadmin.open('新增系列工序','<?php echo url('add'); ?>?series_id='+(select.value('series_id').length>0?select.value('series_id')[0]['value']:''),700, 650);
