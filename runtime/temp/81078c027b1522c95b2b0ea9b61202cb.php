<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:104:"/mnt/datadisk0/www/wwwroot/wchuanghua.ecloudm.com/public/../application/admin/view/series/structure.html";i:1737698138;s:91:"/mnt/datadisk0/www/wwwroot/wchuanghua.ecloudm.com/application/admin/view/public/header.html";i:1635496794;}*/ ?>
<!DOCTYPE html>
...
<style>
    ...
    .check { position: absolute;left:45%;bottom: 40px;}
    .check-box img{margin: 0 auto;display: table-cell;}
    .flower-list img{ margin: 0 auto;display: table-cell;}
    .flower-list input {text-align: center;max-width: 80%;margin-left: 10%;}
    .del-icon{ margin-left: 45%;}
</style>
<body>
        ...
        <div class="layui-row">
            <form class="layui-form" method="post">
                <blockquote class="layui-elem-quote">
                    ...
                    <div class="layui-inline">
                        <select name="level" id="level" lay-filter="selectd">
                ...
                <div class="layui-col-xs12" id="all-flower">
                    ...
                    <div class="layui-col-xs2 check-box" ids='<?php echo $v['id']; ?>'>
                        <img src="/upload/<?php echo $v['structure_pic']; ?>"  height="100"/>
                ...
                <div class="layui-form-item" style="padding-top: 10px;">
                    <blockquote class="layui-elem-quote">
                        ...
                        <select name="sruler_name">
                            <option value="">请选择标尺公式</option>
                        ...
                        <div class="layui-inline" style="margin-top:6px;">
                        <select name="scalculate_name">
                            <option value="">请选择算料公式</option>
                        ...
                        <button class="layui-btn" lay-filter="formula" lay-submit="">全部写入</button>
                    </blockquote>    
                </div>  
                <div class="layui-col-xs12 flower-list">
                    ...
                    <div class='layui-col-xs2'>
                        <img src="/upload/<?php echo $v['structure_pic']; ?>"  height='100'/>
                        <button class="layui-btn layui-btn-normal" type="button" onclick="show_formula(this,<?php echo $v['structure_id']; ?>)" style=" margin-left: 30%">选择公式</button>
                        <input name='ruler_name[]' id="ruler_name" type='text' class='layui-input' placeholder='请选择标尺公式' value='<?php echo $v['ruler_name']; ?>'>
                        <input name='calculate_name[]' id="calculate_name" type='text' class='layui-input'  placeholder='请选择算料公式' value='<?php echo $v['calculate_name']; ?>'>
                        <input name='ruler[]' id="ruler" class="structure" type='hidden' value='<?php echo $v['srf_id']; ?>'/>
                        <input name='calculate[]' id="calculate" class="structure" type='hidden' value='<?php echo $v['scf_id']; ?>'/> 
                        <input name='structure_id[]' type='hidden' value='<?php echo $v['structure_id']; ?>'/>
                        <input name='ss_id[]' type='hidden' value='<?php echo $v['ss_id']; ?>'/>  
    ...
    <script>layui.use(['form', 'layer'],
                function () {
                        ...
                        $('#add').click(function () {
                            ...
                            $('.check-box').each(function () {
                                if ($(this).attr('attr') == 'false') {
                                    ...
                                    var id = $(this).attr('ids');  //id
                                    var pic = $(this).find('img').attr('src'); //图片
                                    var check_html = "<div class='layui-col-xs2'><img src='" + pic + "'  height='100'/>\n\
                                                       <button class='layui-btn layui-btn-normal' type='button' onclick='show_formula(this,"+id+")' style='margin-left: 30%'>选择公式</button>\n\
                                                        <input name='ruler_name[]' id='ruler_name' type='text' class='layui-input' placeholder='标尺公式' value=''>\n\
                                                        <input name='calculate_name[]' id='calculate_name' type='text' class='layui-input'  placeholder='算料公式' value=''>\n\
                                                        <input name='ruler[]' id='ruler' class='structure' type='hidden' value=''/>\n\
                                                        <input name='calculate[]' id='calculate' class='structure' type='hidden' value=''/> \n\
                                                        <input name='structure_id[]' type='hidden' value='"+id+"'/>  \n\
                    ...
                    form.on('select(selectd)',function(data){
                        ...
                        $.post("<?php echo url('series/findStructure'); ?>",{window_type:window,hands:hands,fixed:fixed,level:level},function(obj){
                            var html = '';
                            var array = obj.data;     
                    ...
                    form.on('submit(add)',
                            function (data) {
                                ajaxform("<?php echo url('series/structureAdd'); ?>", data.field, 2);
                                return false;
                            });
                    //写入公式
                    form.on('submit(formula)',
                            function (data) {
                                if(data.field.srulaer_name == '' || data.field.scalculate_name == ''){
                                    layer.msg('请先选择公式',{icon:2});
                                    return false;
                                }
                                ajaxform("<?php echo url('series/writeFormula'); ?>", data.field, 1);
                                return false;
                            });
        ...
        function show_formula(obj,id){
            layer.open({type: 2, content: '/admin/series/selectFormula/structure_id/'+id+'.html',area:['500px','400px'],btn:['确定'],
                shadeClose:true,
