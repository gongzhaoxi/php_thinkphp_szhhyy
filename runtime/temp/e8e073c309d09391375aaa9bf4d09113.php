<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:111:"/mnt/datadisk0/www/wwwroot/wchuanghua2.ecloudm.com/public/../application/admin/view/allorder/edit_delivery.html";i:1635496792;s:92:"/mnt/datadisk0/www/wwwroot/wchuanghua2.ecloudm.com/application/admin/view/public/header.html";i:1635496794;}*/ ?>
<!DOCTYPE html>
...
<body>
        ...
        <div class="layui-row">
            <form class="layui-form">
                ...
                <div class="layui-col-xs12">
                    ...
                    <div class="layui-form-item" style="padding-top: 10px;">
                        <blockquote class="layui-elem-quote">
                            ...
                            <a href="javascript:;" class="layui-btn" id="add" style="margin-top: 6px;"><i class='icon iconfont'>&#xe6b9;</i>添加</a>
                             <input type="radio" name="is_send"  value="0" title="自送" lay-filter="send" <?php if($send['is_send'] == 0): ?>checked<?php endif; ?>>
                            <input type="radio" name="is_send"  value="1" title="物流" lay-filter="send" <?php if($send['is_send'] == 1): ?>checked<?php endif; ?>>
                            <input type="radio" name="is_send"  value="2" title="自提" lay-filter="send" <?php if($send['is_send'] == 2): ?>checked<?php endif; ?>>
                            <input type="radio" name="is_send"  value="3" title="请车" lay-filter="send" <?php if($send['is_send'] == 3): ?>checked<?php endif; ?>>
                            <input type="radio" name="is_send"  value="4" title="快递" lay-filter="send" <?php if($send['is_send'] == 4): ?>checked<?php endif; ?>>
                            <div class="layui-inline" style="float: right">
                                <input type="text" id="send_date" name="send_date" placeholder="送货时间" class="layui-input" lay-verify="required" value="<?php echo $send['send_date']; ?>" style="display:inline;width: 110px;">
                    ...
                    <div class="select-table">
                        ...
                        <tbody id="have_body">
                            ...
                            <tr>
                                <td><input name='sort[]' type='text' value="<?php echo $v['sort']; ?>" class='layui-input sort'></td>
                                <td><?php echo $v['number']; ?></td>
                                <td><?php echo $v['dealer']; ?></td>
                                <td><?php echo $v['address']; ?></td>
                                <td><?php echo $v['count']; ?></td>
                                <td><input type="text" name="arrive_time[]" autocomplete="off" class="layui-input arrive" value='<?php echo $v['arrive_time']; ?>'></td>
                                <td><?php echo round($v['total_price']-$v['have_pay'],2); ?></td>
                                <td>
                                    <button class='layui-btn-danger layui-btn layui-btn-xs' type='button' onclick='order_del(this)' >
                                        <i class='layui-icon'>&#xe640;</i>删除
                                    </button>
                                    <?php if($v['is_send'] == 1): ?>
                                    <button class='layui-btn-warm layui-btn layui-btn-xs' type='button' onclick='order_back(<?php echo $v['id']; ?>)' >
                                        驳回
                                    </button>
                                    <?php endif; ?>
                                </td>
                                <input name='id[]' type='hidden' value="<?php echo $v['id']; ?>"><input name='order_number[]' type='hidden' value="<?php echo $v['number']; ?>">
                                <input name='count[]' type='hidden' value="<?php echo $v['count']; ?>">
                            </tr>
                            <?php endforeach; endif; else: echo "" ;endif; ?>
                        </tbody>
                        </table>
                    ...
                    <div id="driver" style="width: 30%;margin: 0 auto;">
                        <div class="layui-input-inline">
                            <input type="text" name="driver_name" id="driver_name" placeholder="请填写司机姓名" class="layui-input" value='<?php echo $send['driver_name']; ?>'>
                        </div>
                        <div class="layui-input-inline">
                            <input type="text" name="driver_phone" id="driver_phone" placeholder="请填写司机电话" class="layui-input" value="<?php echo $send['driver_phone']; ?>">
                        </div>
                    </div>
                    <div id="logistics" style="display: none;width: 30%;margin: 0 auto;">
                        <div class="layui-input-inline">
                            <input type="text" name="logistics_name" id="logistics_name" placeholder="请填写物流公司名称" class="layui-input" value="<?php echo $send['logistics_name']; ?>">
                        </div>
                        <div class="layui-input-inline">
                            <input type="text" name="logistics_numbers" id="logistics_numbers" placeholder="请填写物流单号" class="layui-input" value="<?php echo $send['logistics_numbers']; ?>">
                        </div>
                    </div>
                </div>  
                ...
                <div class="layui-form-item">
                    ...
                    <input name='send_id' type="hidden" value='<?php echo $send['id']; ?>'/>
    ...
    <script>layui.use(['laydate', 'form', 'layer'],
                function () {
                    ...
                    $(function(){
                        //配送方式输入框显示或隐藏
                        var sendType = "<?php echo $send['is_send']; ?>";
                        if(sendType == 1 || sendType == 4){
                            $('#driver').hide();
                            $('#logistics').show();
                        }else if(sendType == 0 || sendType == 3){
                            $('#driver').show();
                            $('#logistics').hide();
                        }else{
                            $('#driver').hide();
                    ...
                    form.on('submit(add)',
                        function(data) {
                            ...
                            //     }
                            // }
                            ajaxform("<?php echo url('editDelivery'); ?>",data.field);
                            return false;
                        });
                });
        ...
        function order_back(id) {
            layer.confirm('确定要驳回吗?',function () {
                ajaxform("<?php echo url('orderBack'); ?>",{id:id},1);
            })
        }
    </script>
