<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:103:"/mnt/datadisk0/www/wwwroot/wchuanghua2.ecloudm.com/public/../application/admin/view/order/printing.html";i:1769441940;s:92:"/mnt/datadisk0/www/wwwroot/wchuanghua2.ecloudm.com/application/admin/view/public/header.html";i:1635496794;}*/ ?>
<!DOCTYPE html>
				...
				<div class="layui-col-md12">
					<table class="layui-table layui-form" >
							...
							<tr>
								<td colspan="12" style="position: relative;padding-top: 5px;">
									<h3 style="text-align: left;">
										<span id="page" style="font-size: 12px; line-height: 30px;float:left;padding-right:20px ;">&nbsp;</span>	
										<img src='/static/images/logo2023.png?20230703' style="height: 28px; margin-right: 5px; float: left;"/>
										<span style="font-size: 28px; line-height: 30px;">广东恒辉窗花制品有限公司订购清单(标准)</span>
									</h3>
									<div>
						...
						<tbody>
							...
							<tr class="item">
								<td><?php echo $i; ?></td>
								<td style="text-align: left;"><?php echo $v['position']; ?>-<?php echo trim($v['material'],'/'); if($v['flower_type']){ echo '/'.$v['flower_type'];} ?>/<?php echo $v['small']; ?>*<?php echo $v['big'];  $name = unserialize($v['yarn_color']);if($name['name'] != ''){ echo '/网'.$name['name'];} ?></td>
							...
							<tr class="item">
								<td><?php echo $i+count($product); ?></td>
...
<script>
	function print_page(){
		try{
			...
			var newDiv = $(html);
			var arr = [];
			newDiv.find('.item').each(function(){
				arr.push('<tr>'+$(this).html()+'</tr>');
			})
			
			var num = Math.ceil(arr.length/7);
			var index = 0;
			var codeHtml = '';
			for(var i=0;i<num;i++){
				if(i != 0){
					codeHtml = codeHtml + '<div style="page-break-after: always;display: block;"></div>';
				}
				var tmpDiv = $(html);
				tmpDiv.find('.item').remove();
				tmpDiv.find('#page').html('第'+(i+1)+'页/共'+num+'页');
				for(var j=0;j<7;j++){
					var k = i*7 + j;
					tmpDiv.find('#remarkTr').before(arr[k]);
				}
				codeHtml = codeHtml + '<div class="layui-fluid">'+tmpDiv.html()+'</div>';
			}
			
			//var load_index 	= layer.load(2);	
			$('#codeTpl').html(codeHtml);
			$("#codeTpl").print({
				mediaPrint:true,
				timeout:200*num,
